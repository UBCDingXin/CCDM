...
parser.add_argument('--dataset_name', type=str, default='Cell200_badfake')
parser.add_argument('--img_size', type=int, default=128)
parser.add_argument('--filter_rho', type=float, default=0.9)
args = parser.parse_args()


labels = np.arange(1,201,2)
...
for i in trange(len(labels)):
    ...
    for j in range(len(img_lists)):
        ...
        img_ij = Image.open(path_to_img_i_j).convert('L') # 读取图片

        imgData_ij = np.array(img_ij) # 将对象img转化为像素值矩阵
        # imgData_ij = np.transpose(imgData_ij, axes=[2,0,1]) #h,w,c --> c,h,w
        imgData_ij = imgData_ij[np.newaxis,np.newaxis,:,:]
