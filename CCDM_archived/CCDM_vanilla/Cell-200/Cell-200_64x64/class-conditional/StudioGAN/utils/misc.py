...
## my custom image generation function
def generate_images_given_label(data_loader, generator, discriminator, is_generate, num_images, y_sampler, batch_size, z_prior,
                                truncation_factor, z_dim, num_classes, LOSS, OPTIMIZATION, RUN, MODEL, is_stylegan, generator_mapping,
                                generator_synthesis, directory, device):
    ...
    # print("Generate {num_images} {image_type} images from class {y_sampler}.".format(num_images=num_images, image_type=image_type, y_sampler=y_sampler))
    ...
    with torch.no_grad() if not LOSS.apply_lo else dummy_context_mgr() as mpc:
        ...
        images_all = images_all[0:num_images]
        labels_all = labels_all[0:num_images]

        images_all = ((images_all*0.5+0.5)*255.0).astype(np.uint8)
        assert images_all.max()>1 

    # print("Finish dumping images from class {y_sampler} to {directory}".format(y_sampler=y_sampler, directory=directory))
