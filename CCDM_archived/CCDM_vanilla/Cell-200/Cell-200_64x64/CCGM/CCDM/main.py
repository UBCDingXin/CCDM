...
data_filename = args.data_path + '/Cell200_{}x{}.h5'.format(args.image_size, args.image_size)
...
trainset_embedding = IMGs_dataset(images, labels, normalize=True, transform=True, to_neg_one_to_one=True)
...
if args.comp_FID:
    ...
    # ###########################################
    # ''' one h5 files '''
    # print("Start sampling {} fake images for each of {} distinct labels >>>".format(args.nfake_per_label, num_eval_labels))
    # fake_labels = []
    ...
    # ##end for i
    # fake_labels = (np.concatenate(fake_labels)).astype(int)
    
    # dump_fake_images_filename = os.path.join(save_results_folder, 'fake_data_niters{}_nfake{}_{}_sampstep{}.h5'.format(args.niters, int(args.nfake_per_label*num_eval_labels), args.sampler, args.sample_timesteps))
    # if not os.path.isfile(dump_fake_images_filename):
    #     start = timeit.default_timer()
    #     fake_images, _ = trainer.sample_given_labels(given_labels = fn_norm_labels(fake_labels), net_y2h=net_y2h, batch_size = args.samp_batch_size, denorm=True, to_numpy=True, verbose=True, sampler=args.sampler, cond_scale=args.sample_cond_scale, sample_timesteps=args.sample_timesteps, ddim_eta=args.ddim_eta)
    #     stop = timeit.default_timer()
    #     total_sample_time = stop - start
    ...
    #             f.create_dataset('sample_time', data = np.array([total_sample_time]), dtype='float')
    # else:
    #     with h5py.File(dump_fake_images_filename, "r") as f:
    #         fake_images = f['fake_images'][:]
    #         fake_labels = f['fake_labels'][:]
    #         total_sample_time = f['sample_time'][0]
    # assert len(fake_images) == len(fake_labels)
    # ##end if
    # print("\r Got {} fake images for {} distinct labels. Time spent {:.2f}.".format(len(fake_images), num_eval_labels, total_sample_time))
