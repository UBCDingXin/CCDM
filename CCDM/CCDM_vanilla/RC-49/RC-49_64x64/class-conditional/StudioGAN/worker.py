...
class WORKER(object):
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    real_labels = real_labels.type(torch.long)
                    fake_images = fake_images.type(torch.float)
                    fake_labels = fake_labels.type(torch.long)
                    ...
                    ## my custom conversion
                    # real_images_ = real_images_.type(torch.float)
                    # fake_images_ = fake_images_.type(torch.float)
                    # real_labels = real_labels.type(torch.long)
                    # fake_labels = fake_labels.type(torch.long)
    ...
    def generate_fake_images(self, label, nfake_per_label):

        # if self.global_rank == 0:
        #     self.logger.info("Generate {nfake_per_label} fake images for {label} classes.".format(
        #         nfake_per_label=nfake_per_label, label=label))
        if self.gen_ctlr.standing_statistics:
            self.gen_ctlr.std_stat_counter += 1
        ...
        with torch.no_grad() if not requires_grad else misc.dummy_context_mgr() as ctx:
            ...
            fake_images, _ = misc.generate_images_given_label(data_loader=self.eval_dataloader,
                                                                generator=generator,
                                                                discriminator=self.Dis,
                                                                is_generate=True,
                                                                num_images=nfake_per_label,
                                                                y_sampler=int(label),
                                                                batch_size=self.OPTIMIZATION.batch_size,
                                                                z_prior=self.MODEL.z_prior,
                                                                truncation_factor=self.RUN.truncation_factor,
