...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, classes, shape, cond_scale = 6., rescaled_phi = 0.7, save_intermediate=False):
    ...
    @torch.no_grad()
    def sample(self, classes, cond_scale = 6., rescaled_phi = 0.7, save_intermediate=False):
        ...
        return self.p_sample_loop(classes, (batch_size, channels, image_size, image_size), cond_scale, rescaled_phi, save_intermediate)
    ...
    def p_losses(self, x_start, t, *, classes, noise = None, vicinal_weights = None, aux_info = None):
        ...
        # ## use auxiliary net to guide training
        # ## define an elastic loss that allow auxiliary net to make slight prediction error
        # if net_aux is not None and lambda_aux>0 and self.objective == 'pred_noise' and vicinal_weights is not None:
        #     net_aux.eval()
        #     x_t_hat = self.q_sample(x_start = x_start, t = t, noise = model_out) ##use predicted noise to generate x_t from x_0
        #     labels_pred = net_aux(x_t_hat, t) ##pred labels
        #     loss_aux = F.mse_loss(labels_pred.view(-1,1), labels_gt.view(-1,1), reduction = 'none')
        #     aux_weights = torch.ones_like(loss_aux)
        #     aux_weights[null_indx] = 0
        #     num_nonzeros = torch.sum(aux_weights==1).item()
        #     loss_aux = torch.sum(aux_weights.view(-1) * loss_aux.view(-1))/num_nonzeros
        #     # print(loss.item(), loss_aux.item())
        #     loss = loss + lambda_aux*loss_aux
        # ##end if


        ## use auxiliary net to guide training
        ## define an elastic loss that allow auxiliary net to make slight prediction error
        if aux_info is not None and self.objective == 'pred_noise' and vicinal_weights is not None and aux_info["current_step"]>=aux_info["aux_start_step"]: 
            net_aux = aux_info["net_aux"]
            net_aux.eval()
            vicinity_radius = aux_info["vicinity_radius"]
            labels_gt = aux_info["labels_gt"]
            lambda_aux = aux_info["lambda_aux"]

            x_t_hat = self.q_sample(x_start = x_start, t = t, noise = model_out) ##use predicted noise to generate x_t from x_0
            # x_t_hat.clamp_(-1,1)
            labels_pred = net_aux(x_t_hat, t) ##pred labels

            losses_abs = torch.abs(labels_pred.view(-1)-labels_gt.view(-1))
            indx_in_vic = torch.where(losses_abs<=vicinity_radius)[0]
            aux_weights = torch.ones_like(losses_abs)
            aux_weights[null_indx] = 0 #null logits output
            aux_weights[indx_in_vic] = 0 #predicted labels in the vicinity
            indx_nonzero = torch.where(aux_weights==1)[0]
            # loss_aux = torch.sum(aux_weights.view(-1) * (losses_abs**2).view(-1))/len(indx_nonzero)
            loss_aux = torch.sum(aux_weights.view(-1) * losses_abs.view(-1))/len(indx_nonzero)

            # loss_aux = F.mse_loss(labels_pred.view(-1), labels_gt.view(-1), reduction = 'none')
            # indx_in_vic = np.where(np.abs(labels_pred.view(-1).cpu().detach().numpy()-labels_gt.view(-1).cpu().detach().numpy())<=vicinity_radius)[0]
            # aux_weights = torch.ones_like(loss_aux)
            # aux_weights[null_indx] = 0 #null logits output
            # aux_weights[indx_in_vic] = 0 #predicted labels in the vicinity
            # num_nonzeros = torch.sum(aux_weights==1).item()
