...
from utils import cycle, divisible_by, exists, hflip_images, normalize_images, IMGs_dataset
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if self.threshold_type=="hard" and self.kappa==0:
                        ...
                        batch_trainset = IMGs_dataset(batch_images, labels=None, normalize=True, transform = True)
                        batch_dataloader = torch.utils.data.DataLoader(batch_trainset, batch_size=self.batch_size, shuffle=False)
                        batch_dataloader = iter(batch_dataloader)
                        batch_images = next(batch_dataloader)
                        ...
                        with self.accelerator.autocast():
                            loss = self.model(batch_images, classes = net_y2h(batch_labels), vicinal_weights = None)
                    ...
                    ## use the vicinal loss
                        ...
                        with self.accelerator.autocast():
                            loss = self.model(batch_images, classes = net_y2h(batch_target_labels), vicinal_weights = vicinal_weights)
                            loss = loss / self.gradient_accumulate_every
                            total_loss += loss.item()
                    ...
                    if self.step != 0 and divisible_by(self.step, self.sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            gen_imgs = self.ema.ema_model.ddim_sample(
                                        classes = net_y2h(self.y_visual),
                                        shape = (self.y_visual.shape[0], 1, self.image_size, self.image_size),
                                        cond_scale = self.cond_scale_visual,
            ...
            with torch.inference_mode():
                if sampler == "ddpm":
                    ...
                    batch_fake_images = self.ema.ema_model.ddim_sample(
                                        classes = net_y2h(batch_fake_labels),
                                        shape = (batch_fake_labels.shape[0], 1, self.image_size, self.image_size),
                                        cond_scale = cond_scale,
