...
data_filename = args.data_path + '/RC-49_{}x{}.h5'.format(args.image_size, args.image_size)
...
start_label = np.quantile(labels_train, 0.05)
end_label = np.quantile(labels_train, 0.95)
...
if args.comp_FID:
    ...
    assert real_labels.max()>1.0 and fake_labels.max()>1.0
