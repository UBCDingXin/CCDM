...
def load_worker(local_rank, cfgs, gpus_per_node, run_name, hdf5_path):
    ...
    if cfgs.RUN.do_eval:
        ...
        assert real_images.max()>1.0 and fake_images.max()>1.0 ##unnormalized
        ...
        for i in range(len(centers_loc)):
            ...
            # FID
            FID_over_centers[i] = cal_FID(PreNetFID, real_images_curr, fake_images_curr, batch_size=cfgs.RUN.eval_batch_size, resize = None, norm_img = True)
            ...
            # Label score
            labelscores_over_centers[i], _ = cal_labelscore(PreNetLS, fake_images_curr, fake_labels_assigned_curr, min_label_before_shift=0, max_label_after_shift=cfgs.DATA.max_label, batch_size = cfgs.RUN.eval_batch_size, resize = None, norm_img = True)
        ...
        IS, IS_std = inception_score(imgs=fake_images[indx_shuffle_fake], num_classes=5, net=PreNetDiversity, cuda=True, batch_size=cfgs.RUN.eval_batch_size, splits=10, normalize_img=True)
