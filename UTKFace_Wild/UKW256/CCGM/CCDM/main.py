...
data_filename = args.data_path + '/UTKFace_Wild_{}x{}.h5'.format(args.image_size, args.image_size)
...
if args.comp_FID:
    ...
    for i in range(num_eval_labels):
        ...
        if not os.path.isfile(dump_fake_images_filename):
            ...
            fake_images_i, _ = trainer.sample_given_labels(given_labels = fn_norm_labels(fake_labels_i), net_y2h=net_y2h, batch_size = args.samp_batch_size, denorm=True, to_numpy=True, verbose=True, sampler=args.sampler, cond_scale=args.sample_cond_scale, sample_timesteps=args.sample_timesteps, ddim_eta=args.ddim_eta)
