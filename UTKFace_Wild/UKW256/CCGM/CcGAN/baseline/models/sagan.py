...
class sagan_generator(nn.Module):
    ...
    def __init__(self, dim_z, dim_embed=128, nc=3, gene_ch=64):
        ...
        self.snlinear0 = snlinear(in_features=dim_z, out_features=gene_ch*16*8*8)
    ...
    def forward(self, z, labels):
        # n x dim_z
        out = self.snlinear0(z)            # 8*8
        out = out.view(-1, self.gene_ch*16, 8, 8) # 8 x 8
        out = self.block1(out, labels)    # 16 x 16
        out = self.block2(out, labels)    # 32 x 32
        out = self.block3(out, labels)    # 64 x 64
        out = self.self_attn(out)         # 64 x 64
        out = self.block4(out, labels)    # 128 x 128
        out = self.block5(out, labels)    # 256 x 256
        out = self.bn(out)
        out = self.relu(out)
...
if __name__ == "__main__":
    ...
    print('model size G:', get_parameter_number(netG))
    print('model size D:', get_parameter_number(netD))
