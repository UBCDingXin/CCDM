...
# ## dump for training niqe models for filtering
# dump_fake_images_folder = args.path_dump_train_data_for_niqe + '/real_data/RC-49_images_all_0_90/images'
# os.makedirs(dump_fake_images_folder, exist_ok=True)

# for i in trange(len(labels_train_raw)):
#     label_i = labels_train_raw[i]
#     filename_i = dump_fake_images_folder + "/{}_{:.1f}.png".format(i, label_i)
#     os.makedirs(os.path.dirname(filename_i), exist_ok=True)
#     image_i = images_train[i]
...
if args.niqe_filter:
    ...
    save_setting_folder = os.path.join(fake_data_folder, "exp_{}".format(args.setting_name))
...
# target_labels = np.sort(np.array(list(set(labels_all)))) #not normalized
...
## generate bad fake at training labels
target_labels = np.sort(np.array(list(set(labels_train_raw))))
...
if args.niqe_filter:
    ...
    for i in trange(num_target_labels):
        fake_labels_i = target_labels[i]*np.ones([args.niqe_nfake_per_label_burnin,1])
        fake_images_i, _ = fn_sampleGAN_given_labels(fn_norm_labels(fake_labels_i), args.samp_batch_size, to_numpy=True, denorm=True, verbose=False)
        # ### denormalize labels
