...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        ##auxiliary network for predicting label from noisy images
        if lambda_aux>0:
            assert net_aux is not None
            self.net_aux = net_aux 
            self.net_aux.to(self.device)
            self.net_aux.eval()
            self.lambda_aux = lambda_aux
            self.aux_start_step = aux_start_step
        else:
            self.net_aux = None
    ...
    def train(self, net_y2h):
        ...
        ## define necessary information for auxiliary net training
        if self.net_aux is not None and self.lambda_aux>0:
            if self.threshold_type == "hard":
                original_kappa = self.kappa
            else:
                original_kappa = np.sqrt(1/self.kappa)
            aux_info = {
                "net_aux": self.net_aux,
                "lambda_aux": self.lambda_aux,
                "labels_gt": batch_target_labels,
                "vicinity_radius": original_kappa, #we use hard vicinity
                "aux_start_step": self.aux_start_step,
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if self.threshold_type=="hard" and self.kappa==0:
                        ...
                        batch_images = torch.from_numpy(normalize_images(self.train_images[batch_real_indx], to_neg_one_to_one=False))
                        ...
                        with self.accelerator.autocast():
                            loss = self.model(batch_images, classes = net_y2h(batch_labels), vicinal_weights = None, aux_info = aux_info)
                    ...
                    ## use the vicinal loss
                        ...
                        ## define loss
                        with self.accelerator.autocast():
                            loss = self.model(batch_images, classes = net_y2h(batch_target_labels), vicinal_weights = vicinal_weights, aux_info = aux_info)
                            loss = loss / self.gradient_accumulate_every
                            total_loss += loss.item()
                    ...
                    if self.step != 0 and divisible_by(self.step, self.sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            gen_imgs = self.ema.ema_model.ddim_sample(
                                        classes = net_y2h(self.y_visual),
                                        shape = (self.y_visual.shape[0], 3, self.image_size, self.image_size),
                                        cond_scale = 2.0,
    ...
    def sample_given_labels(self, given_labels, net_y2h, batch_size, denorm=True, to_numpy=False, verbose=False, sampler="ddpm", cond_scale=6.0, sample_timesteps=1000, ddim_eta = 0):
        """
        Generate samples based on given labels
        :given_labels: normalized labels
        :net_y2h: label embedding network, pretrained
        """
        accelerator = self.accelerator
        device = accelerator.device
        ...
        ## the pre-trained label embedding network
        net_y2h = net_y2h.to(device)
        net_y2h.eval()
            ...
            with torch.inference_mode():
                if sampler == "ddpm":
                    batch_fake_images = self.ema.ema_model.sample(
                                        classes=net_y2h(batch_fake_labels),
                                        cond_scale = cond_scale
                                        )
                elif sampler == "ddim":
                    batch_fake_images = self.ema.ema_model.ddim_sample(
                                        classes = net_y2h(batch_fake_labels),
                                        shape = (batch_fake_labels.shape[0], 3, self.image_size, self.image_size),
                                        cond_scale = cond_scale,
    ...
    def generate_intermediate_gifs(self, path_to_save, given_labels, net_y2h, fps=20, sampler="ddpm", cond_scale=6.0, sample_timesteps=1000, ddim_eta = 0):
        """
        ...
        with torch.inference_mode():
            if sampler == "ddpm":
                ...
                _, frames = self.ema.ema_model.ddim_sample(
                                    classes = net_y2h(given_labels),
                                    shape = (given_labels.shape[0], 3, self.image_size, self.image_size),
                                    cond_scale = cond_scale,
                                    preset_sampling_timesteps = sample_timesteps,
                                    preset_ddim_sampling_eta = ddim_eta, # 1 for ddpm, 0 for ddim
                                    save_intermediate=True
