...
def load_worker(local_rank, cfgs, gpus_per_node, run_name, hdf5_path):
    ...
    if cfgs.RUN.do_eval:
        ...
        eval_labels = np.arange(cfgs.DATA.min_label, cfgs.DATA.max_label+1) 
        ...
        if cfgs.RUN.dump_fake_for_NIQE:                
            ...
            for i in tqdm(range(len(fake_images))):
                label_i = int(fake_labels_assigned[i])
        ...
        center_start = 1+cfgs.RUN.FID_radius
        center_stop = cfgs.DATA.max_label-cfgs.RUN.FID_radius
        ...
        for i in range(len(centers_loc)):
            ...
            # FID
            FID_over_centers[i] = cal_FID(PreNetFID, real_images_curr, fake_images_curr, batch_size=cfgs.RUN.eval_batch_size, resize = None, norm_img = False)
            # Label score
            labelscores_over_centers[i], _ = cal_labelscore(PreNetLS, fake_images_curr, fake_labels_assigned_curr, min_label_before_shift=0, max_label_after_shift=cfgs.DATA.max_label, batch_size = cfgs.RUN.eval_batch_size, resize = None, norm_img = False)

            print("\r [{}/{}] Center:{}; Real:{}; Fake:{}; FID:{}; LS:{}.".format(i+1, len(centers_loc), center, len(real_images_curr), len(fake_images_curr), FID_over_centers[i], labelscores_over_centers[i]))
        # end for i
        ...
        ls_mean_overall, ls_std_overall = cal_labelscore(PreNetLS, fake_images, fake_labels_assigned, min_label_before_shift=0, max_label_after_shift=cfgs.DATA.max_label, batch_size = cfgs.RUN.eval_batch_size, resize = None, norm_img = True)
        ...
        FID = cal_FID(PreNetFID, real_images[indx_shuffle_real], fake_images[indx_shuffle_fake], batch_size = cfgs.RUN.eval_batch_size, resize = None, norm_img = True)
