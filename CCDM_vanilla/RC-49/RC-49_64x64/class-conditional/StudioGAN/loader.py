...
def load_worker(local_rank, cfgs, gpus_per_node, run_name, hdf5_path):
    ...
    if load_eval_dataset:
        ...
        print("min label: {}; max label: {}".format(min_label_before_shift, max_label_after_shift))
    ...
    if cfgs.RUN.do_eval:
        ...
        PreNetDiversity = ResNet34_class_eval(num_classes=49, ngpu = torch.cuda.device_count()).cuda() #49 types
        ...
        eval_labels = np.sort(np.array(list(set(labels_all)))) #not normalized; generate fake images conditional on these labels
        ...
        ls_mean_overall, ls_std_overall = cal_labelscore(PreNetLS, fake_images, fake_labels_assigned, min_label_before_shift=min_label_before_shift, max_label_after_shift=max_label_after_shift, batch_size = cfgs.RUN.eval_batch_size, resize = None, norm_img = True, num_workers=cfgs.RUN.num_workers)
        ...
        IS, IS_std = inception_score(imgs=fake_images[indx_shuffle_fake], num_classes=49, net=PreNetDiversity, cuda=True, batch_size=cfgs.RUN.eval_batch_size, splits=10, normalize_img=True)
